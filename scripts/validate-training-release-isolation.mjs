...
const PUBLIC_REF='origin/paradise-canvass-manager-public';
const immutable=[
...
// Release work may retain either the frozen field-baseline metadata or the exact validated
// metadata object currently published on paradise-canvass-manager-public. This avoids a
// hand-maintained moving SHA allowlist without accepting arbitrary or rewritten metadata.
...
try{publicLatest=JSON.parse(show(PUBLIC_REF,'latest.json'));}catch{}
const sameMetadata=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
const acceptedMetadata=[baseLatest,...(publicLatest?[publicLatest]:[])];
if(!acceptedMetadata.some(candidate=>sameMetadata(candidate,latest)))throw new Error(`latest.json does not exactly match the frozen baseline or current public validated metadata: ${latest.sha}`);
const allowedLatestVersions=new Set([baseLatest.version,currentVersion,...(publicLatest?[publicLatest.version]:[])]);
...
function assertControlledMetadata(value,label){
  if(value.snapshot!=='2026-08-14'||value.datasetSha256!=='a98b8badf4c3df616fb091eb32ff85f70682f226e4fcd55591f3784b37abe200'||value.records!==78||value.go!==76||value.noGo!==2)throw new Error(`${label} controlled field baseline drift detected`);
  if(value.validated!==true||value.changeControl?.datasetChanged!==false||value.changeControl?.jurisdictionChangeCount!==0||value.changeControl?.classificationChanges!==0)throw new Error(`${label} validation/change-control state is not fail-closed`);
  if(value.url!==`https://rawcdn.githack.com/twiztd1166/proud-math-b7d8/${value.sha}/index.html`)throw new Error(`${label} immutable URL does not match its SHA`);
  if(!Number.isFinite(Date.parse(value.publishedAt)))throw new Error(`${label} publishedAt is invalid`);
}
assertControlledMetadata(latest,'latest.json');
if(publicLatest){
  assertControlledMetadata(publicLatest,'public latest.json');
  try{execFileSync('git',['merge-base','--is-ancestor',publicLatest.sha,PUBLIC_REF],{stdio:'ignore'});}catch{throw new Error(`Current public latest.json SHA is not an ancestor of ${PUBLIC_REF}: ${publicLatest.sha}`);}
}
...
console.log({status:'PASS',base:BASE,publicRef:PUBLIC_REF,publicValidatedSha:publicLatest?.sha||null,immutableFieldFiles:immutable.length,indexException:'Exact additive Training CSS/nav/scripts only',bootException:'Training router only',serviceWorkerException:'Validated field cache + training-only CORE + exact queryless navigation hardening',latestMetadataException:'Frozen baseline or exact current-public validated metadata object only',serviceWorkerTrainingAssets:swTrainingExtras.length,trainingAdded,totalDiffEntries:diff.length});
