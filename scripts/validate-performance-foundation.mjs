...
requireText(sql, /performance_location_insert_own/i, 'Own-only GPS mutation policy missing');
requireText(sql, /performance_sets_read_all/i, 'Team Set visibility policy missing');
requireText(sql, /performance_sets_update_authorized/i, 'Set mutation authorization policy missing');
...
if (/insert\s+into\s+public\.performance_kpi_standard_versions/i.test(sql)) fail('Migration must not seed invented KPI standards');
if (/insert\s+into\s+public\.performance_pay_plan_versions/i.test(sql)) fail('Migration must not seed invented pay rules');
