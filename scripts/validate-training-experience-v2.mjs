...
const index=req('index.html'),exp=req('training-experience-v2.js'),polish=req('training-ux-polish-v1.js'),practiceEngine=req('training-practice-v1.js'),practice=req('training-practice-data-v2.js'),deep=req('training-deep-audit-fixes-v1.js'),sw=req('sw.js'),build=req('scripts/build-canvass-site.mjs');
const fail=m=>{throw new Error(m)};
if(!index.includes('<script src="training-experience-v2.js"></script>'))fail('training-experience-v2.js not loaded');
if(!index.includes('<script src="training-ux-polish-v1.js"></script>'))fail('training-ux-polish-v1.js not loaded');
...
for(const marker of ['2026.08.17-pu-training-ux-polish-v1','puTrainingNavV1','NEXT MILESTONE','puTrainingMilestone','puTrainingNavStack'])if(!polish.includes(marker))fail(`missing UX polish marker ${marker}`);
if(!polish.includes('requestAnimationFrame(()=>window.scrollTo(0,y))'))fail('context-preserving scroll restore missing');
for(const marker of ['2026.08.17-pu-practice-v3-adaptive','Practice My Weak Areas','puPracticeWeakScenarioIds','NEEDS_PRACTICE'])if(!practiceEngine.includes(marker))fail(`missing adaptive practice marker ${marker}`);
