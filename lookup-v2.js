...
function city(){
  let r=sel,s=status(r),fav=isFavorite(r.name),block=currentDeployBlock(),age=typeof pcmSnapshotAgeDays==='function'?pcmSnapshotAgeDays():null,hoursBlocker=hoursTextBlocker(r),releaseBlocked=r.release!=='GO'||!!block||hoursBlocker;
  ...
  ${hoursBlocker?`<div class="blockBanner"><b>HOURS TEXT BLOCKER — COMMERCIAL CANVASS HOLD.</b> Legal classification remains GO, but no commercial route release may produce DEPLOY until the controlled hours text is resolved. Do not invent or infer a time window.</div>`:''}
  ...
  document.getElementById('rel').onclick=()=>{if(block){toast(block);return}if(hoursBlocker){toast('HOURS TEXT BLOCKER: commercial canvass release is on hold until the controlled hours text is resolved.');return}if(r.release==='GO'){releaseStep=0;setView('release')}else toast('COMMERCIAL CANVASS NO-GO: do not deploy this ordinary canvass route. Courtesy-notice controls, if any, are separate.')};
