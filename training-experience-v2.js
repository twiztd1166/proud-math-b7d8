(()=>{
  ...
  const apprenticeOrder=['sales-apprentice-intro','sales-process-map','sales-shadowing','sales-apprentice-ready'];
  const apprenticeKeep=new Set(apprenticeOrder);
  const hiddenApprentice=new Set(PU_LESSONS.filter(x=>x.stage==='sales-apprentice'&&!apprenticeKeep.has(x.id)).map(x=>x.id));
  ...
  const stageStats=id=>{const xs=PU_LESSONS.filter(x=>x.stage===id),done=xs.filter(ready).length;return{done,total:xs.length,pct:xs.length?Math.round(done/xs.length*100):0}};
  const compactStage=()=>{
    const next=nextCore();
    if(next){const i=trackOrder.indexOf(next.stage);return{current:stageName(next.stage),next:trackOrder[i+1]?stageName(trackOrder[i+1]):'Canvasser complete'}}
  ...
  const section=(title,items,empty='')=>`<div class="puSection">${esc(title)}</div><section class="card puMediaPlaylist" data-playlist="${esc(title)}">${items.length?items.map(puMediaCard).join(''):`<div class="puEmpty">${esc(empty)}</div>`}</section>`;
  ...
  // Default employee queue is the job they need first: Foundation -> Field Ready -> Canvasser.
  // Future-role curriculum remains explorable in Career Path and does not silently become required.
  ...
  puStageStatus=function(stage){
    ...
    const done=lessons.filter(ready).length;
  ...
  function stage(stageId){
    ...
    if(stageId==='sales-apprentice'){
      ...
      const marker=[...M.querySelectorAll('.puSection')].find(x=>(x.textContent||'').trim()==='Lessons');if(marker)marker.textContent='Bridge lessons';
      ...
      const guide=M.querySelector('.puCareerGuide');if(guide){
        guide.querySelectorAll('.row').forEach(row=>{const lab=(row.querySelector('.lab')?.textContent||'').trim(),val=row.querySelector('.val');if(!val)return;if(lab==='SKILLS LEARNED')val.textContent='Role boundary, full sales-process map, supervised shadowing, and readiness for formal Sales Rep training.';if(lab==='WHAT’S NEXT'||lab==="WHAT'S NEXT")val.textContent='Sales Rep Academy for the detailed in-home process.'});
      }
    }
    if(stageId==='sales-rep'){
      const sections=[...M.querySelectorAll('.puSection')],lessonSection=sections.find(x=>/Core sales lessons/i.test(x.textContent||''));
      if(lessonSection){
        ...
        if(firstAfter){
          ...
          const body=details.querySelector('.puSalesControlsBody');let n=firstAfter;while(n){const next=n.nextSibling;body.appendChild(n);n=next}M.appendChild(details);
        }
      }
    }
  }
  ...
  function lesson(id){
    baseLesson(id);
    if(view!=='training')return;
    const lessonObj=PU_LESSONS.find(x=>x.id===id)||(window.PU_CONTENT?.managerLessons||[]).find(x=>x.id===id);if(!lessonObj)return;
    const mediaStep=[...M.querySelectorAll('.puLessonStep')].find(x=>(x.querySelector('small')?.textContent||'').trim()==='WATCH / LISTEN');if(mediaStep&&(!lessonObj.media||!lessonObj.media.length))mediaStep.remove();
    const quick=M.querySelector('.puQuickCheck'),actions=M.querySelector('.puLessonActions');if(quick&&actions){const passStep=actions.closest('.puLessonStep');if(passStep)passStep.insertBefore(quick,actions)}
    const doneBtn=document.getElementById('puDone'),nextBtn=document.getElementById('puNext'),checkRequired=typeof puQuickCheckRequired==='function'&&puQuickCheckRequired(id);
    const sync=()=>{
    ...
    if(nextBtn&&trackOrder.includes(lessonObj.stage)){
      ...
      if(n){nextBtn.textContent='NEXT LESSON';nextBtn.onclick=()=>puSetPage('lesson:'+n.id)}else if(lessonObj.stage===CORE_TRACK_END){nextBtn.textContent='VIEW PROGRESS';nextBtn.onclick=()=>puSetPage('progress')}
    }
    if(nextBtn&&apprenticeKeep.has(id)){
      const i=apprenticeOrder.indexOf(id),n=apprenticeOrder[i+1];
      if(n){nextBtn.textContent='NEXT BRIDGE LESSON';nextBtn.onclick=()=>puSetPage('lesson:'+n)}else{nextBtn.textContent='VIEW CAREER PATH';nextBtn.onclick=()=>puSetPage('career')}
    }
  }

  function filterSearch(){
    M.querySelectorAll('[data-result-type="lesson"]').forEach(b=>{if(hiddenApprentice.has(b.dataset.resultId))b.remove()});
  }
  ...
  renderTraining=function(){
    if(view!=='training')return;
    if(puPage==='home')return puHome();if(puPage==='media')return puMedia();if(puPage==='progress')return puProgress();if(puPage==='career')return puCareer();if(puPage.startsWith('stage:'))return puStage(puPage.slice(6));if(puPage.startsWith('lesson:'))return puLesson(puPage.slice(7));
    const out=baseRender();
    if(puPage==='search'){
      const input=document.getElementById('puSearchInput');if(input){input.addEventListener('input',()=>setTimeout(filterSearch,0));setTimeout(filterSearch,0)}
    }
    return out;
  };
  ...
  window.PU_SALES_APPRENTICE_BRIDGE_IDS=Object.freeze(apprenticeOrder.slice());
  window.PU_HIDDEN_DUPLICATE_LESSON_IDS=Object.freeze([...hiddenApprentice]);
})();