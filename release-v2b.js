...
function sourceProof(r){const refs=pcmAuthorityRefs(r),src=r.sources||[];return`${refs.length?`<div class="authorityRefs"><b>Code sections referenced:</b> ${refs.map(esc).join(' · ')}</div>`:''}${src.map((u,i)=>`<div class="sourceRef">${esc(pcmSourceDescriptor(u,i))}</div><a class="source" href="${esc(u)}" target="_blank" rel="noopener">Open official source ${i+1}</a>`).join('')}${!navigator.onLine?'<div class="offlineSourceNote">You are offline. The app instructions are still available; official source pages need an internet connection.</div>':''}`}
function fieldResponseCard(){
  ...
  return`<section class="card fieldResponse"><h3>What to do now</h3><p>${esc(now)}</p><div class="responseLabel">Next step</div><p>Correct the issue or get manager/compliance clearance, then start a new Daily Check before canvassing.</p><div class="responseLabel">If someone questions the rule</div><p>${esc(challengeSummary())}</p><div class="responseLabel">Official source</div>${sourceProof(sel)}</section>`;
}
function releaseFieldAnswers(){
  return`<section class="card essentials finalFieldAnswers"><div class="sectionTitle">FIELD ANSWERS — KEEP THIS SCREEN OPEN</div>${fieldDetailRow('Hours',hoursSummary(sel))}${fieldDetailRow('Door hanger',hangerActionSummary(sel))}${fieldDetailRow('Courtesy notice',courtesyActionSummary(sel))}${fieldDetailRow('Always','OBEY POSTED SIGNS. LEAVE IF ASKED. NEVER USE A USPS MAILBOX.')}</section>`;
}
...
function release(){
  ...
  if(hoursTextBlocker(sel)){
    M.innerHTML=`<div class="head"><div><h2>Daily Check</h2><p>${esc(sel.name)}</p></div><button class="back" id="cityBack">Back</button></div><section class="traffic stop"><div class="trafficSymbol">!</div><div><small>COMMERCIAL ROUTE</small><h3>HOURS NOT CLEARED</h3><p>Do not start a commercial canvass route here until Compliance confirms the hours.</p></div></section><section class="card essentials">${row('Hours',hoursSummary(sel),'strong')}</section><section class="card fieldResponse"><h3>What to do now</h3><p>Do not canvass this route yet. Do not guess or borrow hours from another city. Wait until Compliance clears the hours and the app is updated.</p><div class="responseLabel">Door hanger and courtesy notice</div><p>Those rules are separate and remain shown on the city page.</p><div class="responseLabel">Official source</div>${sourceProof(sel)}</section>`;
    bindCityBack();return
  }
  if(sel.release!=='GO'){
    let s=status(sel);M.innerHTML=`<div class="head"><div><h2>Daily Check</h2><p>${esc(sel.name)}</p></div><button class="back" id="cityBack">Back</button></div><section class="traffic stop"><div class="trafficSymbol">✕</div><div><small>CANVASSING</small><h3>${esc(s.label)}</h3><p>${esc(managerAction(sel))}</p></div></section><section class="card fieldResponse"><h3>What to do now</h3><p>Do not canvass this route.</p><div class="responseLabel">Why?</div><p>${esc(whySummary(sel))}</p><div class="responseLabel">If someone questions the rule</div><p>${esc(challengeSummary())}</p><div class="responseLabel">Official source</div>${sourceProof(sel)}</section>`;bindCityBack();return
  }
