...
function liveDecisionCompareBoard(profiles,open=false){
  ...
  const body=rows.map(p=>{
    ...
    const blockers=String(review.blockers_text||'No governed blocker recorded').trim();
    ...
    return `<tr data-live-compare-row="${esc(p.profile_id)}">
      ...
      <td data-label="Readiness" data-live-compare-readiness-profile="${esc(p.profile_id)}"><div class="liveCompareReadiness"><b>${esc(readiness)}</b><span>Still needed: ${esc(blockers)}</span></div></td>
  ...
  return `<details class="liveCompareBoard" data-live-decision-comparison ${open?'open':''}>
    <summary class="liveCompareHead"><div><span>Live opportunity comparison</span><b>${rows.length} governed targets</b></div><small>Decision · date · current + prior cost · historical outcome · current + best placement · contact · readiness + still needed · hard deadline · timing · direct action</small></summary>
