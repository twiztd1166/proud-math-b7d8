...
function resolutionLaneLabel(value){
  const v=String(value||'').toUpperCase();
  return ({
    PARADISE_ACTION:'Paradise action required',
    ORGANIZER_RESPONSE:'Organizer response required',
    WAIT_FOR_PUBLICATION:'Wait for publication',
    RECONCILE_EXISTING:'Reconcile existing commitment',
    RESEARCH_REQUIRED:'Research required'
  })[v]||String(value||'Unclassified').replaceAll('_',' ');
}
...
function rebookReviewCard(review,hasLiveOpportunity=false){
  ...
  const resolution=review.resolution_lane?`<div data-resolution-lane-profile="${esc(review.profile_id)}"><span>Resolution lane</span><b>${esc(resolutionLaneLabel(review.resolution_lane))}</b></div>`:'';
