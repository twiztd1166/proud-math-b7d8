...
function sourceLabeledPerformanceValue(row){
...
function historyPerformanceStateValue(row){
  ...
  if(sourceLabeled)return 'SOURCE-LABELED PERFORMANCE — '+sourceLabeled;
...
function boothOutcomeRows(rows){
  return (rows||[]).filter(row=>{
    const booth=historyBoothValue(row);
    if(sourceSemanticState(booth)!=='VALUE')return false;
    return ['issued_appts','demos','net_sales_count','net_revenue','gross_sales_count','gross_sales_value','nsli']
      .some(field=>row?.[field]!==null&&row?.[field]!==undefined&&String(row[field]).trim()!=='');
  }).sort((a,b)=>{
    const netA=Number(a.net_revenue??-1),netB=Number(b.net_revenue??-1);
    if(netA!==netB)return netB-netA;
    const salesA=Number(a.net_sales_count??-1),salesB=Number(b.net_sales_count??-1);
    ...
    return Number(b.issued_appts??-1)-Number(a.issued_appts??-1)||Number(b.source_year||0)-Number(a.source_year||0);
  });
}
function boothOutcomeMetrics(row){
  const parts=[];
  if(row?.net_revenue!==null&&row?.net_revenue!==undefined)parts.push(money(row.net_revenue)+' net');
  if(row?.net_sales_count!==null&&row?.net_sales_count!==undefined)parts.push(String(row.net_sales_count)+' net sale'+(Number(row.net_sales_count)===1?'':'s'));
  if(row?.com_percent!==null&&row?.com_percent!==undefined)parts.push(String(row.com_percent)+'% COM');
  if(row?.issued_appts!==null&&row?.issued_appts!==undefined)parts.push(String(row.issued_appts)+' issued');
...
function yearScorecard(year,rows,lpItems,cumulativeItems,cleanupChecklist=''){
  ...
  const sourceLabeledPerformance=rows.some(row=>!historyPerformancePresent(row)&&sourceLabeledPerformanceValue(row))
    ?`<div class="yearField full"><span>Source-labeled performance (not reclassified)</span><div>${datedValues(rows,row=>!historyPerformancePresent(row)?sourceLabeledPerformanceValue(row):'',v=>esc(v))}</div></div>`
    :'';
...
async function openCatalog(id,focusYear=null){
  ...
  try{
    ...
    const seriesRelationHtml=canonicalSeriesRelation
      ?`<div class="rebookSeries"><span>Same organizer series</span><b>Series decision target is tracked under ${esc(canonicalSeriesRelation.canonical_series_profile_id)}. This profile remains preserved only for source-grain history and lifetime evidence; do not treat it as a second booking target.</b><div class="actions"><button class="btn secondary catalogSeriesBtn" data-profile="${esc(canonicalSeriesRelation.canonical_series_profile_id)}">Open current series profile</button></div></div>`
      :(retainedSeriesRelations.length?`<div class="rebookSeries"><span>Organizer-series identity</span><b>Current opportunity is anchored here. Legacy source profile${retainedSeriesRelations.length===1?'':'s'} ${retainedSeriesRelations.map(esc).join(' · ')} remain preserved for provenance and are not separate booking targets.</b></div>`:'');
    ...
    const liveHistoricalDecisionEvidenceHtml=catalogProfile&&typeof historicalDecisionEvidence==='function'?historicalDecisionEvidence(catalogProfile):'';
    const liveHistoricalOutcomeHtml=catalogProfile&&typeof historicalOutcomeSnapshot==='function'?historicalOutcomeSnapshot(catalogProfile):'';
    const liveHistoricalPlacementHtml=catalogProfile&&typeof historicalPlacementGuide==='function'?historicalPlacementGuide(catalogProfile):'';
...
function openDetail(id){
  ...
  const profile=typeof currentProfileForShow==='function'?currentProfileForShow(s):null;
...
$$('.nav button').forEach(b=>b.onclick=()=>{state.deepLinkedProfile=null;state.deepLinkedYear=null;state.tab=b.dataset.tab;syncLocationView();render();if(state.tab==='shows'&&state.showMode!=='UNLINKED'&&!state.catalogLoaded&&!state.catalogLoading)loadCatalog();if(state.tab==='shows'&&state.showMode==='UNLINKED'&&!state.unlinkedLp.loaded&&!state.unlinkedLp.loading)loadUnlinkedLp();window.scrollTo(0,0)});
