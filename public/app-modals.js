...
function yearScorecard(year,rows,lpItems){
...
async function openCatalog(id){
  ...
  try{
    const d=await call('catalogHistory',{profileId:id}),p=d.profile,h=d.history||[],perf=d.performance||[],mfcs=Array.isArray(p.matched_mfc_ids)?p.matched_mfc_ids:[];
    ...
    const years=[...new Set([...h.map(x=>Number(x.source_year||0)),...perf.map(x=>Number(x.source_year||0))].filter(Number.isFinite).filter(Boolean))].sort((a,b)=>b-a);
    ...
    const historyHtml=years.length?orderedYears.map((year,i)=>{
      ...
      return `<details class="historyYear ${focusedHistoryYear===year?'focusedYear':''}" ${i===0?'open':''}><summary><span>${esc(year)}${focusedHistoryYear===year?'<span class="yearFocusTag">Selected year</span>':''}</span><span>${rows.length} record${rows.length===1?'':'s'} · ${lpItems.length} LP</span></summary><div class="historyYearBody">${yearScorecard(year,rows,lpItems)}</div></details>`;
