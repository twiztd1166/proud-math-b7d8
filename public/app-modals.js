...
function annualLpMetric(x,directKey,canonicalKey,fallback=null){
  const fields=x&&x.source_fields&&typeof x.source_fields==='object'?x.source_fields:{};
  const direct=fields[directKey];
  if(direct!==null&&direct!==undefined&&String(direct)!=='')return direct;
  const totals=fields.annual_report&&fields.annual_report.canonical_annual_totals&&typeof fields.annual_report.canonical_annual_totals==='object'?fields.annual_report.canonical_annual_totals:{};
  const canonical=totals[canonicalKey];
  if(canonical!==null&&canonical!==undefined&&String(canonical)!=='')return canonical;
  return fallback;
}
function lpMetricText(value){return value===null||value===undefined||String(value)===''?'—':esc(value)}
function lpYearHtml(items){
  if(!items.length)return '<div class="yearLpEmpty">No annual-period LeadPerfection performance source is linked to this year.</div>';
  return items.map(x=>{
    const reports=(Array.isArray(x.report_months)?x.report_months:[]).filter(m=>m.gmail_url);
    const sourceUrl=x&&x.source_fields&&x.source_fields.source_report_url?x.source_fields.source_report_url:'';
    const reportHtml=reports.length?`<details class="sourceReports"><summary>Source reports · ${reports.length} month${reports.length===1?'':'s'}</summary><div class="reportLinkList">${reports.map(m=>`<a class="btn secondary sourceBtn" target="_blank" href="${esc(m.gmail_url)}">Open ${esc(m.month||'LP report')}</a>`).join('')}</div></details>`:sourceUrl?`<div class="reportLinkList"><a class="btn secondary sourceBtn" target="_blank" href="${esc(sourceUrl)}">Open annual LP report</a></div>`:'';
    const raw=annualLpMetric(x,'raw','raw');
    const setCount=annualLpMetric(x,'set','set',x.issued);
    const issue=annualLpMetric(x,'issue','issue',x.opportunities);
    const demos=annualLpMetric(x,'demo','demo',x.demos);
    const gross=annualLpMetric(x,'gross_close_count','gross_close',x.close_sales_count);
    const grossVolume=annualLpMetric(x,'gross_close_volume','gross_sale',x.close_sales_volume);
    const net=annualLpMetric(x,'net_close_count','net_close');
    const netVolume=annualLpMetric(x,'net_sale_volume','net_sale');
    return `<div class="yearLpCard"><div class="yearLpHead"><b>${esc(x.source_label||'LeadPerfection')}</b><span>${esc(String(x.match_status||'').replaceAll('_',' '))}</span></div><div class="yearMetricGrid compact"><div><span>Raw</span><b>${lpMetricText(raw)}</b></div><div><span>Set</span><b>${lpMetricText(setCount)}</b></div><div><span>Issue</span><b>${lpMetricText(issue)}</b></div><div><span>Demo</span><b>${lpMetricText(demos)}</b></div><div><span>Gross closes</span><b>${lpMetricText(gross)}</b></div><div><span>Gross volume</span><b>${money(grossVolume)}</b></div><div><span>Net closes</span><b>${lpMetricText(net)}</b></div><div><span>Net volume</span><b>${money(netVolume)}</b></div></div>${x.verification_status?`<div class="yearVerification">${esc(x.verification_status)}</div>`:''}${reportHtml}</div>`;
  }).join('');
}
function cumulativeYearHtml(items){
  if(!items.length)return '';
  return items.map(x=>{
    const status=String(x.comparison_status||'').replaceAll('_',' ');
    const sourceLink=x.source_report_url?`<div class="reportLinkList"><a class="btn secondary sourceBtn" target="_blank" href="${esc(x.source_report_url)}">Open cumulative LP report</a></div>`:'';
    return `<div class="yearLpCard cumulativeLpCard"><div class="yearLpHead"><b>${esc(x.source_label||'LeadPerfection cumulative')}</b><span>${esc(status)}</span></div><div class="yearMetricGrid compact"><div><span>Raw</span><b>${esc(x.raw_count??'—')}</b></div><div><span>Set</span><b>${esc(x.set_count??'—')}</b></div><div><span>Issue</span><b>${esc(x.issue_count??'—')}</b></div><div><span>Demo</span><b>${esc(x.demo_count??'—')}</b></div><div><span>Gross closes</span><b>${esc(x.gross_close_count??'—')}</b></div><div><span>Gross volume</span><b>${money(x.gross_close_volume)}</b></div><div><span>Net closes</span><b>${esc(x.net_close_count??'—')}</b></div><div><span>Net volume</span><b>${money(x.net_close_volume)}</b></div></div><div class="yearVerification">2012–2026 cumulative re-query attribution · separate from annual-period performance · not attendance proof.</div>${sourceLink}</div>`;
  }).join('');
}
...
function yearScorecard(year,rows,lpItems,cumItems=[]){
  const recordCount=rows.length;
  const lpCount=lpItems.length;
  const cumulativeCount=cumItems.length;
...
async function openCatalog(id){
  ...
  try{
    ...
    const lifetimeSource=sourceRows.map(x=>`<a class="btn secondary sourceBtn" target="_blank" href="https://docs.google.com/spreadsheets/d/${encodeURIComponent(x.spreadsheet_id)}/edit#gid=977393722&range=A${Number(x.row)}:R${Number(x.row)}">Lifetime source row ${Number(x.row)}</a>`).join('');
    ...
    const historyHtml=years.length?orderedYears.map((year,i)=>{
      ...
      const cumItems=cum.filter(x=>Number(x.source_year||0)===year);
      return `<details class="historyYear ${focusedHistoryYear===year?'focusedYear':''}" ${i===0?'open':''}><summary><span>${esc(year)}${focusedHistoryYear===year?'<span class="yearFocusTag">Selected year</span>':''}</span><span>${rows.length} record${rows.length===1?'':'s'} · ${lpItems.length} annual LP${cumItems.length?` · ${cumItems.length} cumulative LP`:''}</span></summary><div class="historyYearBody">${yearScorecard(year,rows,lpItems,cumItems)}</div></details>`;
    ...
    $('#detailBody').innerHTML=`<h2>${esc(p.canonical_event)}</h2><div class="subtitle">${esc(p.profile_id)} · ${esc(p.tier||p.source_type.replaceAll('_',' '))}</div><div class="detailGrid"><div class="detail"><div class="k">Lifetime occurrences</div><div class="val">${esc(p.occurrences??'—')}</div></div><div class="detail"><div class="k">Preserved history records</div><div class="val">${h.length}</div></div><div class="detail"><div class="k">Lifetime net</div><div class="val">${money(p.lifetime_net_volume)}</div></div><div class="detail"><div class="k">Lifetime net sales</div><div class="val">${esc(p.lifetime_net_sales??'—')}</div></div><div class="detail"><div class="k">Lifetime close volume</div><div class="val">${money(p.lifetime_close_volume)}</div></div><div class="detail"><div class="k">Annual LP sources</div><div class="val">${perf.length}</div></div><div class="detail"><div class="k">Cumulative LP sources</div><div class="val">${cum.length}</div></div></div>${latestContactHtml}${aliases.length?`<div class="block"><div class="k">Known aliases</div><div class="val">${aliases.map(esc).join(' · ')}</div></div>`:''}${mfcs.length?`<div class="block"><div class="k">Current operating control</div><div class="actions">${mfcs.map(x=>`<button class="btn primary catalogMfcBtn" data-mfc="${esc(x)}">Open ${esc(x)}</button>`).join('')}</div></div>`:''}${lifetimeSource?`<div class="block"><div class="k">Lifetime performance source</div><div class="actions">${lifetimeSource}</div></div>`:''}<div class="sourceWarn historyIntro"><b>Year cards are source-first.</b> Missing fields are shown explicitly instead of being hidden. Annual-period LeadPerfection and the 2012–2026 cumulative re-query are displayed as separate evidence layers; cumulative values do not replace annual values, and neither layer is treated as attendance proof.</div>${operatingHtml?`<div class="sectionTitle"><span>Current operating year${operatingRows.length===1?'':'s'}</span><span>${operatingRows.length} control${operatingRows.length===1?'':'s'}</span></div><div class="historyList currentYearList">${operatingHtml}</div>`:''}${historyFocusNote}<div class="sectionTitle"><span>Preserved historical years</span><span>${years.length} year${years.length===1?'':'s'}</span></div><div class="historyList">${historyHtml}</div><div class="actions"><button class="btn secondary" id="catalogCloseBtn">Close</button></div>`;
...
$$('.nav button').forEach(b=>b.onclick=()=>{state.tab=b.dataset.tab;render();if(state.tab==='shows'&&state.showMode==='ALL'&&!state.catalogLoaded&&!state.catalogLoading)loadCatalog();window.scrollTo(0,0)});
$('#refreshBtn').onclick=async()=>{toast('Reloading operating data…');await bootstrap();if(state.catalogLoaded)await loadCatalog(true);toast('Current')};
