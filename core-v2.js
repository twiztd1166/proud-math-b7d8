...
function addressSummary(r){let a=String(r?.addressCheck||'').trim();if(/EXACT ADDRESS|LEGAL JURISDICTION CHECK REQUIRED/i.test(a))return'Confirm the exact city or county for the address before using this rule.';return'Confirm the address is inside the city or service-area jurisdiction shown.'}
function refusalSummary(r){let s=String(r?.refusal||'').trim();if(!s)return'Honor posted property signs and any resident refusal. Leave immediately if asked.';if(/No Soliciting|No Trespassing|refusal|leave immediately|asks? .*leave/i.test(s))return'Honor all No Soliciting, No Advertising, No Trespassing, and similar property notices that apply. If a resident says no or asks you to leave, stop and leave immediately.';return'Honor posted property signs, resident instructions, and any direct refusal. Leave immediately if asked.'}
function materialsSummary(){return'Use only lawful private-property access. Never bypass a gate, fence, private street, access control, property sign, or resident instruction.'}
...
function hangerActionSummary(r){
  ...
  if(m.includes('RECEPTACLE'))return'Use an existing non-USPS flyer or newspaper holder, or hand it directly to a willing resident. Do not attach it to the home.';
...
function hangerWhereSummary(r){
  ...
  if(m.includes('SPECIAL LOCATION')||m.includes('ADDRESS SPLIT'))return'Check the exact city/county and read More door-hanger details before leaving anything.';
  if(m.includes('HANG ON FRONT KNOB')||m.includes('HANG ON KNOB'))return'Front-door knob or handle only where the rule allows it. Keep it secure and removable.';
  ...
  return firstSentence(r?.hangerPlacement||'Follow the placement rule shown below.');
}
...
function courtesyWhereSummary(r){
  ...
  if(a.includes('KNOB/HANDLE'))return'Front-door knob or handle only where the rule allows it; keep it secure and removable.';
  ...
  return firstSentence(r?.courtesyPlacement||'Follow the placement rule shown below.');
}
function hangerSignsSummary(){return'Do not leave literature where a No Soliciting, No Advertising, No Trespassing, or similar notice applies, or after any direct refusal. Follow any stricter city-specific rule.'}
function publicPlacementSummary(){return'Do not leave literature on streets, sidewalks, rights-of-way, parks, vehicles, poles, or other public surfaces. A direct handoff is allowed only where the city-specific rule permits it.'}
...
function courtesyContentSummary(){return'Installation-day information only. No sales pitch, estimate or inspection offer, discount, financing, quote request, sales link or QR code, referral offer, testimonial, or cross-sell.'}
function whySummary(r){if(r?.release==='NO-GO')return'This route is not approved for ordinary residential canvassing under the city rule shown in the source record. Do not use a different placement or time as a workaround.';if(hoursTextBlocker(r))return'The city has solicitation rules, but the current hours wording has not been verified well enough to give the field a safe time window. Commercial canvassing stays on hold until Compliance clears it.';return'This area is approved for appointment-setting only under the conditions shown in this app. The GO status does not override property signs, access rules, door-hanger restrictions, or the separate rules that apply if the conversation becomes a sale.'}
function plainScript(r){if(r?.release==='NO-GO')return'Paradise does not canvass this route. I’m going to stop and have my manager verify any question.';if(hoursTextBlocker(r))return'This route is not cleared for commercial canvassing yet because the local hours are still under review.';return'We’re Paradise Exteriors. We’re only setting a future appointment today. We’re not taking an order, signing a contract, collecting money, or giving a price at the door.'}
function challengeSummary(){return'If a resident, HOA, security officer, city employee, or law-enforcement officer objects, stop. Do not argue. Note who raised the issue, where and when it happened, and what they said. Contact manager/compliance before resuming.'}
