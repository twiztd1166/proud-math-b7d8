...
test('employee can Continue Training through a lesson when third-party media is on rights hold',async({page})=>{
  ...
  const media=page.locator('.puMediaCard').filter({hasText:'Canvassing Welcome Onboarding'}).first();await expect(media).toBeVisible();await expect(media.getByRole('button',{name:'RIGHTS REVIEW'})).toBeDisabled();await expect(media).toContainText(/No explicit Paradise internal reproduction \/ re-hosting grant/i);
...
test('knowledge-gated lesson requires content plus Quick Check but not rights-held reference media',async({page})=>{
  ...
  const media=page.locator('.puMediaCard').filter({hasText:'Formula For Handling Objections'}).first();await expect(media).toBeVisible();await expect(media.getByRole('button',{name:'RIGHTS REVIEW'})).toBeDisabled();
