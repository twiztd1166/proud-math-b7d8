...
test('lesson completion persists on the device',async({page})=>{
  ...
  await page.reload();await page.locator('#nTrain').click();await page.getByRole('button',{name:/My Progress/}).first().click();await expect(page.getByText('1 complete')).toBeVisible();
});
