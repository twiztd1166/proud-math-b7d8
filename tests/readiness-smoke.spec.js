...
test('My Progress puts the current canvasser track first and keeps certification evidence secondary',async({page})=>{
  ...
  const details=page.locator('details').filter({hasText:/Advancement & certification details/});await expect(details).toBeVisible();await details.locator(':scope > summary').click();
...
test('content marked complete does not satisfy a required knowledge gate',async({page})=>{
  ...
  await expect(page.getByText(/0 of \d+ gates/)).toBeVisible();await expect(page.getByText(/Quick Check pending/i)).toBeVisible();
...
test('Career Path guidance covers every stage and preserves sales and manager boundaries',async({page})=>{
  ...
  await page.locator('#puBack').click();await page.getByRole('button',{name:/6\. Sales Rep/}).click();guide=page.locator('.puCareerGuide');await expect(guide).toContainText(/Full Sales Rep certification is not available from Part 1 alone/i);const controls=page.locator('details.puSalesControls');await controls.locator(':scope > summary').click();await expect(page.getByText(/CURRENT POLICY REQUIRED/)).toBeVisible();
