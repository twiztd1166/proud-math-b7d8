...
test('core lookup remains usable across the device matrix',async({page})=>{
  await page.goto('/index.html');await pick(page,'Boynton Beach');
...
test('all 78 records resolve to explicit canvass, hours and placement answers',async({page})=>{
  ...
  const audit=await page.evaluate(()=>window.PCM_DATA.records.map(r=>({name:r.name,release:r.release,decision:fieldCanvass(r).title,hoursRaw:String(r.hours||''),hours:fieldHours(r),hangerMode:String(r.hangerMode||''),courtesyMode:String(r.courtesyMode||''),hanger:fieldHanger(r),courtesy:fieldCourtesy(r)})));
  ...
  expect(await page.evaluate(()=>Object.keys(FIELD_HOURS_OVERRIDES).sort())).toEqual(overrideNames);
  expect(audit.filter(x=>x.release==='GO'&&!/HOURS TEXT BLOCKER/i.test(x.hoursRaw)&&!overrideNames.includes(x.name)&&x.hours!=='Use Paradise’s normal route schedule.')).toEqual([]);
  expect(audit.filter(x=>['Deerfield Beach','Hudson','Land O Lakes','Lutz','Palmetto','Spring Hill','Wesley Chapel'].includes(x.name)&&/7:00 AM|8:00 PM|9:00 AM|10:00 PM/.test(x.hours))).toEqual([]);
  ...
  expect(byName['Bradenton'].courtesy).toBe('HANDOFF WITH CONSENT — do not leave it unattended.');
  expect(byName['Fort Pierce'].courtesy).toBe('HANDOFF ONLY — do not leave it unattended.');
