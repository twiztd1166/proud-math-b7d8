...
test('service worker daily1 only refreshes offline shell from exact queryless app entry',async({page})=>{
  await page.goto('/index.html');
  const sw=await page.evaluate(()=>fetch('/sw.js').then(r=>r.text()));
  expect(sw).toMatch(/const CACHE='[^']*trainingux5-experience3-redteam7-daily1';/);
  ...
  expect(sw).toContain("'./training-daily-v1.js'");
  expect(sw).toContain("'./training-daily-v1.css'");
