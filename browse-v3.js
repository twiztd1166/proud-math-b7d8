...
function browse(){
  ...
  M.innerHTML=`<div class="head"><div><h2>Browse Areas</h2><p>Choose a county, then a municipality or service area.</p></div><button class="back" id="browseBack">Search</button></div><div class="countyGrid">${groups.map(([c,a])=>{let no=a.filter(r=>r.release==='NO-GO').length;return`<button class="countyBtn" data-county="${esc(c)}"><b>${esc(c)} County</b><small>${a.length} areas${no?` · ${no} NO-GO`:''}</small></button>`}).join('')}</div><div class="notice"><b>Address still controls.</b> County browsing helps locate the controlled row; exact legal jurisdiction / route confirmation is still required before DEPLOY.</div>`;
